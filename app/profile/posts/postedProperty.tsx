...
const PropertyPostsList = () => {
  ...
  const loadPosts = async () => {
    try {
      const userPosts = await getUserPostsByCategory(user!.uid, "Property")
      setPosts(userPosts)
    } catch (error) {
                ...
                <View className="flex-row">
                  ...
                  <View className="flex-1">
                    ...
                    <Text className="text-white/50 text-sm">
                      {post.location} {post.bedrooms && `• ${post.bedrooms} beds`}
                    </Text>
                  </View>
                </View>
...
export default PropertyPostsList